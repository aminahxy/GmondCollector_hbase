...
<%
        ...
        Map<String, Map<Object,Map<String,Map<String,Map<String,Object>>>>> res =
                new GetClusterMetaInstanceInService().displayClusterMetaInstanceWithState(clusterName);
        ...
        * */
        for (Map.Entry<String, Map<Object,Map<String,Map<String,Map<String,Object>>>>> entry : res.entrySet() ) {
            String mnode = entry.getKey();
            String matrix_server = mnode.split(":")[0];
            String service = mnode.split(":")[3];
            if ( mnode.split(":").length == 6 ) {
                service = mnode.split(":")[4];
            } 
             ...
             */
            for(Map.Entry<Object,Map<String,Map<String,Map<String,Object>>>> entry1: entry.getValue().entrySet()){
                ...
                if( !entry1.getValue().isEmpty()){
                    if(entry1.getValue().containsKey("RUNNING")){
                        ...
                        for(Map.Entry<String,Map<String,Object>> entry2: entry1.getValue().get("RUNNING").entrySet()){
                            //System.out.println(entry2.getKey() + ":" + entry2.getValue());
                            sb_running_hosts.append("<p>" + "\n"
                                       + entry2.getKey() + ":" + entry2.getValue().get("State") 
                                       + ":" + entry2.getValue().get("ErrorCode")  + "\n"
                                       + "</p>" + "\n");
                            running_num++;
                    ...
                    if(entry1.getValue().containsKey("ERROR")){
                        //System.out.println(entry1.getValue().get("RUNNING").getClass().toString());
                        for(Map.Entry<String,Map<String,Object>> entry2: entry1.getValue().get("ERROR").entrySet()){
                            //System.out.println(entry2.getKey() + ":" + entry2.getValue());
